...
public class WishlistBean {
	...
	public void addToWishlist(WishList wishlist) {
		...
		String query = "INSERT INTO wishlist " + "(product, username) " + "VALUES ('" + newProductName + "', '"
				+ newCustomerName + "');";
		try {
	...
	/*
	 * public Category getCategoryByName(String name) { String query =
	 * "SELECT * FROM category WHERE categoryName = '" + name + "';";
	 * List<List<String>> categoryQueryList = null; String categoryName = null;
	 * Category category = null;
	 * 
	 * try { categoryQueryList = connectionBean.executeBeanQuery(query); for(int
	 * i = 0; i < categoryQueryList.size(); i++) { categoryName =
	 * categoryQueryList.get(i).get(0); category = new Category(categoryName);
	 * return category; } } catch(Exception e) { e.printStackTrace(); } return
	 * null; }
	 */
}